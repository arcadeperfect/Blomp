...
//     private static void SolveCubic(out int nRoots, out float x1, out float x2, out float x3, float a, float b, float c,
//         float d)
//     {
...
//             var Saturation = CubeRoot(R + SQRT_D);
//             var T = CubeRoot(R - SQRT_D);
//             x1 = Saturation + T - a_over_3;
//             x2 = float.NaN;
//             x3 = float.NaN;
//         }
//
...
//     //HuiInput: parentRotation and parentPosition: the current parent transform.
//     //HuiInput: startParentRotation and startParentPosition: the transform of the parent object at the time the objects are parented.
//     //HuiInput: startChildRotation and startChildPosition: the transform of the child object at the time the objects are parented.
...
// // #else
// // 		currentCamera = Camera.main;
// // 		mousePosition = HuiInput.mousePosition;
// // #endif
// //
...
//     //HuiInput: position. If the output is used for motion simulation, the input transform
...
//     //HuiInput: rotation (quaternion). If the output is used for motion simulation, the input transform
//     //has to be located at the seat base, not at the vehicle CG. Attach an empty GameObject
//     //at the correct location and use that as the input for this function.
//     //A low number of samples can give a jittery result due to rounding errors.
//     //If more samples are used, the output is more smooth but has a higher latency.
